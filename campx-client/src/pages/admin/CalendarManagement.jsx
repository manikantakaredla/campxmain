...
const CalendarManagement = () => {
  ...
  const fetchActivities = async () => {
    try {
      const response = await api.get('/calendar', { params: { limit: 200 } })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Event Details Modal with Delete Button */}
      ...
      {showDeleteModal && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-2xl z-50 w-full max-w-md p-5">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete "<span className="font-medium">{showDeleteModal.title}</span>"? This action cannot be undone.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteModal(null)} className="flex-1 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={() => handleDelete(showDeleteModal._id)} className="flex-1 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
                Delete
              </button>
            </div>
          </div>
