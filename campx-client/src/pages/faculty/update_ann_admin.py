...
submit_append = """      submitData.append('targetMySection', formData.targetMySection)
...
content = content.replace("      submitData.append('targetMySection', formData.targetMySection)\n      submitData.append('targetMyDepartment', formData.targetMyDepartment)", submit_append)


# 5. Advanced Targeting UI conditionally rendered based on role
advanced_targeting_ui = """
        {/* Audience Target Flags conditionally rendered */}
...
# Try to replace the old audience target flags
# Need a robust regex since we added it using a python script last time
pattern_flags = re.compile(r'\{\/\* Audience Target Flags \*\/\}.*?<\/div>.*?(?=\{\/\* Attachment & Calendar \*\/|\Z)', re.MULTILINE | re.DOTALL)
content = pattern_flags.sub(advanced_targeting_ui + '\n        ', content)

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Updated CreateAnnouncement.jsx for admin targeting")
